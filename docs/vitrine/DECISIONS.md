...
## ADR-020 — Le profil d'entreprise : ce que l'employé sait de son client, et par où ça passe
**Date :** 2026-08-06
**Contexte :** le briefing d'après-achat ne retenait que deux chaînes (`criteria`, `offer`) — exactement les deux champs du formulaire qu'il remplaçait. Tout le reste de ce que le client disait (son métier, ses preuves, les objections qu'il entend, qui ne jamais contacter, son ton, ce qu'il ne promet jamais) était collecté par la conversation puis jeté à l'extraction, faute d'un endroit où l'écrire.

**Défaut réel corrigé au passage :** `loadIdentity` retournait `config.systemPrompt` et s'arrêtait là. Or tout client passé par le chat d'accueil (`platform.create_tenant_agent`, ADR-019) EN A un — donc pour **tous les vrais clients**, la configuration écrite par le formulaire comme par le briefing n'atteignait jamais l'employé. Elle était en base, relue par personne. Un employé configuré qui travaille comme s'il ne l'était pas est pire qu'un employé non configuré : personne ne va chercher la panne. Le prompt d'accueil et le profil s'ADDITIONNENT désormais (`composeSystemPrompt`).

**Décision :** un profil à huit clés (`activite`, `cible`, `offre`, `preuves`, `objections`, `exclusions`, `ton`, `interdits`), dont deux seulement sont exigées — `cible` et `offre`, sans lesquelles l'employé ne peut pas travailler. Les six autres sont retenues **si le client les dit**, jamais réclamées pour elles-mêmes : un interrogatoire en huit questions après l'achat coûterait plus qu'il ne rapporte.

**Forme plate, choisie pour la convergence :** une clé = une chaîne = une future ligne `company_profile` du schéma du cœur (`key text`, `value jsonb`, `author 'client'`). Le jour où les deux schémas convergeront, il n'y aura aucune traduction à écrire. D'ici là le profil vit dans `agent_instance.config.companyProfile`.

**Écriture fusionnée, jamais remplacée :** corriger la cible depuis le formulaire à deux champs ne doit pas effacer les objections et les interdits recueillis par le briefing. `saveCompanyProfile` fusionne en jsonb. Les clés historiques `prospectingCriteria`/`prospectingOffer` restent écrites en doublon — la route cron sélectionne les employés dessus (`config ? 'prospectingCriteria'`), les retirer débrancherait la prospection.

**Ce qui rend tout ça vérifiable (ADR-0024 du cœur) :** un test bloque toute divergence entre les champs de `BRIEFING_TOOL` et ceux du profil — un champ demandé au client puis silencieusement jeté est la panne la plus coûteuse de ce module. Et une suite d'intégration (`apps/vitrine/src/lib/company-briefing.integration.test.ts`) tient le chemin complet sur un vrai Postgres, base jetable et migrations réappliquées : ce que le client dit → `agent_instance.config` → le prompt que l'employé relit. Le job `schema` de la CI la fait tourner sur une base `vitrine_test` séparée, `SENTIO_REQUIRE_DB_TESTS=1` pour qu'elle ne puisse pas sauter en silence.

**Ce qui reste ouvert :** les deux schémas (cœur et vitrine) modélisent toujours la même chose deux fois. La convergence — migrer la vitrine sur le schéma du cœur, et donc sur le vrai `company_profile` — est décidée dans son principe mais pas planifiée. Aucun pont entre les deux bases ne sera construit d'ici là : il figerait la duplication au lieu de la résorber.
