...
| 2026-08-27 | ⚠️ **Le diagnostic public était MUET, pour deux causes empilées** | 1. Groq a retiré `llama-3.3-70b-versatile` ; 2. `provider_quota` refusait d'écrire, sa clé étrangère pointant vers un `provider_credential` VIDE. La seconde n'est apparue qu'après la première |
| 2026-08-27 | **Le nom du modèle sort du code** (`SENTIO_GROQ_MODELES`) | Un fournisseur retire ses modèles sans prévenir : ce n'est pas un incident, c'est le marché. Le corriger doit prendre des minutes, pas une soirée |
| 2026-08-27 | **Lady apprend à POSER une question**, pas seulement quoi recueillir | Sept règles : reprendre ses mots, une seule question, ne jamais redemander ce qu'on peut déduire, et chaque question existe parce que la réponse change l'employé |
...
| 2026-08-28 | **Audit avant production** (`docs/35`) + correction des trois constats P0 | Sentio était **bien construit et mal branché** : rien ne remplissait `lead`, rien n'appelait le battement, et le client se voyait attribuer des capacités que le runtime refuse. ⚠️ Un client qui aurait payé n'aurait rien obtenu — sans erreur, sans message : un espace fonctionnel où il ne se passe rien |
