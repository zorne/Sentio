...
- **Supabase (Postgres, région Union européenne)** : base, authentification, isolation par
  ligne, et planification interne. Une seule dépendance pour quatre besoins.
- **Hébergement de l'interface** : à choisir en gardant l'application **indépendante de
  l'hébergeur** — aucune interface propriétaire, car une migration est probable dès le
  premier client payant (voir [`11-exploitation.md`](11-exploitation.md)).
