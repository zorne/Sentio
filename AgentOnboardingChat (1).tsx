  ...
  return (
    <div className="agent-chat">
      ...
      {messages.length > 3 && (
        <button
          type="button"
          className="agent-chat__fallback"
          onClick={() => {
            // Secours si le backend n'a jamais renvoyé done:true (ex: le
            // system prompt de buildConfigureSystemPrompt n'a pas encore
            // été mis à jour côté lib/knowledge-base.ts). On construit un
            // agent minimal à partir de ce qu'on a déjà reçu, à défaut de
            // patch complet.
            setAgent((prev) => prev ?? { id: agentId, name: "Votre employé IA", role: "commercial" });
            setPhase("revealing");
          }}
        >
          J'ai terminé, montre-moi mon agent →
        </button>
      )}
